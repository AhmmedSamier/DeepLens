...
## 2026-11-12 - [Fast Path Property Early-Exit]
**Learning:** Checking `itemBitflags` is an effective early exit for overall matches, and `itemNameBitflags`/`itemFullNameBitflags` work for specific properties. However, when evaluating the fallback path via `tryFuzzyMatchPath`, the evaluation proceeds directly to expensive `Fuzzysort.single()` operations on the path string. Missing an explicit `itemPathBitflags` check results in wasted fuzzy search cycles if the query characters match the aggregate bitflags but are missing from the path itself.
**Action:** Isolate and maintain an `itemPathBitflags` array in hot paths, parallel to the existing bitflag arrays. Use this specific bitmask to implement an O(1) early-exit check inside `tryFuzzyMatchPath` before executing expensive string evaluations.
