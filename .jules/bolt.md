...
## 2024-05-30 - [O(1) early-exits inside helper functions in hot loops]
**Learning:** When applying early exits to O(N) hot loops, placing the early exit condition *inside* a helper function (like `tryFuzzyMatchName`) still incurs function call overhead for every item.
**Action:** Move simple boolean or bitmask early-exit checks to the very top of the calling loop method (like `calculateFuzzyScore`) to skip function calls entirely for non-matching items.
