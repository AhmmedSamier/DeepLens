...
## 2026-12-15 - [Avoid O(n) early-exits inside helper functions in hot loops for property matches]
**Learning:** Even if we have property-specific bitflag arrays, calling helper functions like `tryFuzzyMatchName` only to immediately return `-Infinity` based on an O(1) bitflag check inside the helper function still incurs function call overhead for every rejected item.
**Action:** Move the property-specific bitmask checks (`itemNameBitflags`, `itemFullNameBitflags`, `itemPathBitflags`) outside of the helper functions, directly into the calling loop method (`calculateFuzzyScore`), to skip function calls entirely for non-matching items.
