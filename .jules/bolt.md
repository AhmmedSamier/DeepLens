...
## 2024-06-25 - [Fast Scalar Return in Hot Loops]
**Learning:** To avoid memory overhead from unnecessary object allocations when evaluating endpoints in hot paths (like `SearchEngine`), do not return `{ score, scope } | null` from functions called inside the loop.
**Action:** Refactor functions like `tryUrlEndpointMatch` and `calculateUrlMatchScore` to return scalar values (e.g., a simple `number` or `-Infinity`) and handle the scope logic at the call site. This eliminates object creation overhead and GC pauses.
