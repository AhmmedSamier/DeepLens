...
## 2026-10-28 - [Hoist Allocations to Class Properties in Hot Paths]
**Learning:** Creating intermediate collections (like `Set` and `Uint8Array`) inside hot path methods like `searchRemainingItems` causes unnecessary garbage collection overhead on every search query.
**Action:** Hoist these temporary collections into reusable class-level properties (e.g., `reusablePriorityTypeIds`). Clear and reuse them per call, which is safe due to the single-threaded Node.js environment and `_isSearching` locks.
