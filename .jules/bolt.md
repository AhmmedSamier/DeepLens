...
## 2026-11-10 - [Avoid Object Allocation in Hot Loops]
**Learning:** In hot loops, returning an object structure like `{ score, scope } | null` causes unnecessary memory allocations resulting in garbage collection overhead. Since `scope` is statically known in URL endpoint matching (`SearchScope.ENDPOINTS`), the `scope` assignment can be handled at the call site.
**Action:** Refactor hot loop evaluation functions like `tryUrlEndpointMatch` to return a primitive scalar `number` representing the score, using `-Infinity` for failed matches. This avoids object instantiation overhead completely.
