...
## 2026-08-10 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** When falling back to `tryFuzzyMatchPath`, the evaluation proceeds directly to expensive `Fuzzysort.single()` string operations. Missing an explicit `relativeFilePath` bitflag check results in wasted fuzzy search cycles.
