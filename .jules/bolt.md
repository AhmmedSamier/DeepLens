...
## 2026-11-01 - [O(1) Property Fuzzy Match Early-Exit]
**Learning:** In `SearchEngine.calculateFuzzyScore`, the O(1) early-exit checks for property-specific fuzzy matching (`tryFuzzyMatchName`, `tryFuzzyMatchFullName`, `tryFuzzyMatchPath`) were placed *inside* those helper functions. This means that for every item, we were incurring function call overhead only to immediately return `-Infinity`.
**Action:** Move the property bitmask checks (`context.itemNameBitflags`, etc.) outside the function calls into `calculateFuzzyScore`. By wrapping the function calls in `if` statements that check the bitmasks, we avoid function call overhead for items that are immediately rejected, significantly improving hot loop performance.
