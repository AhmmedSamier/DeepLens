...
## 2025-03-01 - Optimize String Indentation Calculation
**Learning:** In V8 hot paths, using `String.prototype.search(/\S|$/)` introduces noticeable regex engine overhead. Calculating indentation via `line.length - line.trimStart().length` is faster as it leverages V8 native string methods.
**Action:** Use native string methods like `trimStart()` over regex for simple operations in hot loops.
