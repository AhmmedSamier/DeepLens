...
## 2024-05-30 - O(1) Path Property Early-Exit in Fuzzy Match
**Learning:** Similar to name and fullName, checking the aggregate `itemBitflags` is an effective early exit for overall matches. However, when falling back to `tryFuzzyMatchPath`, the evaluation proceeds directly to expensive `Fuzzysort.single()` string operations. If the query characters matched the aggregate bitflags purely due to the `name` or `fullName`, missing an explicit `path` bitflag check results in wasted fuzzy search cycles.
