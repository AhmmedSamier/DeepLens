...
## 2026-09-03 - [Fast URL Match Early-Exit]
**Learning:** In V8 (Node.js/Bun) hot loops, returning newly allocated objects (e.g., `{ score, scope } | null`) from functions called inside the loop incurs significant memory churn and garbage collection overhead.
**Action:** Refactor functions like `tryUrlEndpointMatch` to return scalar values (e.g., a simple `number` or `-Infinity`) and handle the scope logic directly at the call site.
