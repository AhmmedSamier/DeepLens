...
## 2024-07-12 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** Similar to name and full name matching, skipping the path property early-exit check results in wasted Fuzzysort.single() string operations when falling back to tryFuzzyMatchPath. This happens when query characters matched against the aggregate itemBitflags because they were spread across other properties, but not the path itself.
