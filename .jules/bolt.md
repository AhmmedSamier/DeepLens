...
## 2024-05-24 - Add Path Bitflags for Fuzzy Search Early Exit
**Learning:** Similar to the `name` and `fullName` property early-exits, checking `itemPathBitflags` provides an effective O(1) early exit for path matches. When falling back to `tryFuzzyMatchPath`, the evaluation proceeded directly to expensive `Fuzzysort.single()` string operations without verifying if the path even contained the characters first.
**Action:** Implement `itemPathBitflags` as a parallel array to store pre-computed bitflags for `relativeFilePath`. Check this bitflag in `tryFuzzyMatchPath` before performing fuzzy sorting, bypassing it entirely if the path doesn't contain all required characters.
