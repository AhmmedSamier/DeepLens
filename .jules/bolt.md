...
## 2026-04-09 - Pre-computed Lowercase Strings in Hot Paths
...
**Action:** In `SearchEngine` scoring loops, avoid calling `.toLowerCase()` on item properties. Instead, retrieve pre-computed lowercased strings from parallel Fuzzysort prepared arrays (e.g., `this.preparedFullNames[i]`) using `targetLower` to eliminate redundant string allocations.
