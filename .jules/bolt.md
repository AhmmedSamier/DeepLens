...
## 2026-08-27 - [Avoid object allocation in hot loops]
**Learning:** To avoid memory churn and garbage collection overhead in hot paths, avoid returning newly allocated objects (e.g., `{ score, scope } | null`) from functions called inside the loop.
**Action:** Refactor these functions to return scalar values (e.g., a simple `number` or `-Infinity`) and handle the scope logic at the call site.
