...
## 2026-08-27 - Optimize string indentation calculations
**Learning:** Using String.prototype.search(/\S|$/) to calculate indentation introduces regex engine overhead, which can be a performance bottleneck in hot paths. Using line.length - line.trimStart().length leverages fast V8 native methods and maintains full Unicode/BOM compatibility.
**Action:** Always prefer .trimStart() length calculations over regex matching for leading whitespace in V8 environments.
