...
## 2026-12-14 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** Missing an explicit bitflag check for the `path` property during `tryFuzzyMatchPath` evaluation results in wasted `Fuzzysort.single()` evaluations, because the query characters might only exist across the `name` or `fullName`, leading to false positives on the aggregate `itemBitflags` check.
**Action:** Isolate and maintain an `itemPathBitflags` array in parallel to `itemNameBitflags`, `itemFullNameBitflags` and `itemBitflags`. Use this specific bitmask to implement an O(1) early-exit check inside `tryFuzzyMatchPath` before executing expensive string evaluations.
