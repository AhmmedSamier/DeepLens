...
**Learning:** Similar to the name and fullName properties, falling back to `tryFuzzyMatchPath` proceeds directly to expensive `Fuzzysort.single()` string operations. Since `relativeFilePath` has its own characters, and the aggregate `itemBitflags` might pass due to characters in other properties (like name), missing an explicit path bitflag check results in wasted fuzzy search cycles.
