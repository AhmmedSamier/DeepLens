...
## 2024-05-28 - [Class-level reusable buffers for hot paths]
**Learning:** In synchronous hot paths that process search results sequentially, repeatedly allocating new objects like `new Set()` or `new Uint8Array(256)` causes unnecessary memory overhead and garbage collection.
**Action:** Allocate class-level reusable buffers (e.g. `reusablePriorityTypeIds`, `burstUrlMatchIdsCache`) once and reuse them by clearing or resetting them at the start of the method instead of allocating new instances during each call.
