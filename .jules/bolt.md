...
## 2024-05-28 - Fast Scalar Returns in Hot Path Function Calls
**Learning:** Returning scalar values instead of object allocations inside hot loops helps avoid massive memory churn and garbage collector overhead. In `tryUrlEndpointMatch`, returning an object containing `score` and `scope` slowed down the loop.
**Action:** Change hot-path functions (like `tryUrlEndpointMatch` and `calculateUrlMatchScore`) to return simple scalars (like `-Infinity` or `number`).
