...
## 2024-05-30 - [Fast String Indentation Calculation]
**Learning:** When optimizing string indentation calculations in hot paths, using `String.prototype.search(/\S|$/)` incurs unnecessary regex engine overhead.
**Action:** Use `line.length - line.trimStart().length` instead. This leverages fast V8 native string trimming methods and maintains full Unicode/BOM compatibility while being significantly faster than regex or manual loops.
