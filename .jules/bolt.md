...
## 2026-10-31 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** Checking `itemPathBitflags` is an effective early exit for `relativeFilePath` matches. When falling back to `tryFuzzyMatchPath`, the evaluation proceeded directly to expensive `Fuzzysort.single()` string operations. Missing an explicit `path` bitflag check results in wasted fuzzy search cycles.
