...
## 2026-08-14 - [Return scalars instead of objects in hot loops]
**Learning:** Returning objects from functions inside a hot loop, even if perfectly predictable and simple, creates measurable memory allocation and garbage collection overhead. In the case of `tryUrlEndpointMatch`, returning `{ score, scope } | null` was allocating an object purely for a scalar tuple update.
**Action:** Always return a single scalar type, like a `number` representing a score, and rely on standard flags like `-Infinity` to signify failure. Manage side states like `scope` adjustments directly in the caller if they are statically known.
