...
## 2024-08-31 - [Optimize object allocation in hot loops]
**Learning:** In V8 (Node.js/Bun), returning a newly allocated object like `{ score: number, scope: SearchScope }` from a frequently called function in a hot loop triggers significant garbage collection pressure. It can drastically increase execution time.
**Action:** Refactor hot loop helpers to return scalar primitive values (like `number`) instead of objects, and handle any necessary scope/state assignments manually at the call site.
