...
## 2026-09-02 - Fast String Optimizations
**Learning:** String.prototype.search(/\S|$/) is slow in hot paths due to regex engine overhead. A while(indexOf) loop is also suboptimal for counting newlines in large strings in V8.
**Action:** Use string.length - string.trimStart().length for indentation calculation, and use lastIndexOf + charCodeAt loop for counting newlines in large strings.
