...
## 2024-05-28 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** Checking `itemBitflags` is an effective early exit for aggregate matches, but when falling back to `tryFuzzyMatchPath`, the evaluation proceeds directly to expensive `Fuzzysort.single()` string operations. Since characters might be spread purely across the `name` or `fullName` (passing the `itemBitflags` aggregate check but not existing in `relativeFilePath`), missing an explicit path bitflag check results in wasted fuzzy search cycles.
**Action:** Isolate and maintain an `itemPathBitflags` array in hot loops, parallel to `itemNameBitflags` and `itemFullNameBitflags`. Use this specific bitmask to implement an O(1) early-exit check inside `tryFuzzyMatchPath` before executing expensive string evaluations.
