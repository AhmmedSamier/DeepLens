...
## 2026-10-31 - [Optimize Fallback Iteration and Personalization]
**Learning:** In hot methods like `searchRemainingItems`, repeatedly allocating small arrays like `new Uint8Array(256)` incurs unnecessary GC pressure. Also, in `applyPersonalizedBoosting`, calculating `1 - weight` within a loop over all results wastes CPU cycles.
**Action:** Precompute static lookup tables at the module level when possible, and extract loop-invariant calculations outside of loops.
