...
## 2026-11-12 - [Fast String Indentation Calculation]
**Learning:** In text parsing hot paths (like calculating indentation in `processSingleLine`), using regex `buffer.search(/\S|$/)` to find the first non-whitespace character incurs significant regex engine overhead.
**Action:** Replace `search(/\S|$/)` with `line.length - line.trimStart().length`. This leverages fast V8 native methods for string manipulation, handles Unicode whitespace robustly, and avoids regex overhead entirely.
