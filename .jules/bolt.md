...
## 2024-05-29 - [Eliminate Object Allocations in URL Hot Paths]
**Learning:** Returning dynamically allocated objects like `{ score, scope }` or `{ score }` from helper methods (`tryUrlEndpointMatch`, `calculateUrlMatchScore`) called within high-frequency hot loops (`processItemForSearch`) introduces significant GC pressure and unnecessary memory overhead. This allocation happens for thousands of endpoints repeatedly, bogging down V8 engine performance.
**Action:** Refactor hot loop helper methods to always return a primitive scalar value (like a `number` for the score, using `-Infinity` to indicate a miss). Hardcode known metadata (like `SearchScope.ENDPOINTS`) directly at the caller site to completely eliminate object instantiations.
