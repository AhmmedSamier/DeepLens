...
## 2026-09-05 - [Fast String Indentation Calculation]
**Learning:** When calculating string indentation in hot paths, avoiding `String.prototype.search(/\S|$/)` (regex engine overhead) and manual `charCodeAt(i) <= 32` loops (misses Unicode whitespace, slower than native C++ optimizations) is crucial. Instead, `line.length - line.trimStart().length` leverages fast V8 native methods and maintains full Unicode/BOM compatibility.
**Action:** Use `line.length - line.trimStart().length` to calculate string indentation in performance-critical sections to eliminate regex evaluation overhead and avoid garbage collection overhead.
