...
**Learning:** Checking `itemNameBitflags` and `itemFullNameBitflags` provides effective early exits for `name` and `fullName` matches. However, when falling back to `tryFuzzyMatchPath`, the evaluation proceeds directly to expensive `Fuzzysort.single()` string operations without checking if the query characters actually exist in the path. Missing an explicit `path` bitflag check results in wasted fuzzy search cycles for items that passed the aggregate bitmask due to characters existing in other fields.
**Action:** Isolate and maintain an `itemPathBitflags` array in hot paths, parallel to `itemBitflags` and others. Use this specific bitmask to implement an O(1) early-exit check inside `tryFuzzyMatchPath` before executing expensive string evaluations.
