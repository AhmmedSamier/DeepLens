...
## 2026-08-26 - Inline Fuzzy Searching
**Learning:** The previous attempt to hoist bitflag checks manually using ternary operators degraded performance.
**Action:** Manually inline the entire fuzzy search logic (`tryFuzzyMatchName`, etc.) directly into `calculateSearchScore` to fully eliminate the 4 layers of function call overhead (`calculateSearchScore` -> `calculateFuzzyScore` -> `tryFuzzyMatch...`) in the hot loop.
