...
## 2026-11-20 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** Checking `itemBitflags` is an effective aggregate early exit, but when falling back to `tryFuzzyMatchPath`, the evaluation proceeds directly to expensive `Fuzzysort.single()` string operations. Since path can contain characters spread differently from `name` or `fullName`, missing an explicit `path` bitflag check results in wasted fuzzy search cycles when the characters exist in the item but not in the path itself.
**Action:** Isolate and maintain an `itemPathBitflags` array in hot paths, parallel to `itemNameBitflags`, `itemFullNameBitflags` and `itemBitflags`. Use this specific bitmask to implement an O(1) early-exit check inside `tryFuzzyMatchPath` before executing expensive string evaluations.
