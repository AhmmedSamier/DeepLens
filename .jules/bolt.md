...
## 2026-10-27 - [Scalar Return Types for Hot Loops]
**Learning:** To avoid memory churn and garbage collection overhead in hot paths, returning newly allocated objects (e.g., `{ score, scope } | null`) from functions called inside the loop creates unnecessary object allocations per iteration.
**Action:** Refactor functions in hot loops (like `tryUrlEndpointMatch` and `calculateUrlMatchScore`) to return scalar values (e.g., a simple `number` or `-Infinity`) and handle the metadata logic (like `scope`) at the call site.
