...
## 2024-07-02 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** Even when failing the `name` and `fullName` bitflag checks and falling back to `tryFuzzyMatchPath`, the fallback path executes expensive `Fuzzysort.single()` operations directly. Since the required characters may not be present in the path at all (passing the `itemBitflags` aggregate check but due to other fields), missing an explicit `path` bitflag check results in wasted fuzzy search cycles.
