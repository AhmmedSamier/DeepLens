...
## 2026-04-09 - [Fast Lazy Precomputed Object Lowercase in Search]
**Learning:** In frequently executed hot loops (like `burstSearch` iterating over potentially 100,000+ items), unconditionally calling `.toLowerCase()` on secondary string properties (e.g., `fullName`) incurs significant string allocation and CPU overhead, even when the property is not strictly needed for the final score.
**Action:** Eliminate redundant string allocations by reusing the precomputed lowercased string from the `Fuzzysort.Prepared` object (`_targetLower`) where available. Furthermore, delay its resolution by passing the precomputed object to the scoring function so it is only extracted or computed if fast-path primary match conditions (like exact or prefix matches on `name`) fail. This reduces runtime for 100,000 iterations from ~28ms to ~17ms (Bun 1.2.14, Linux x86_64, 4-core, 8GB RAM).

## 2026-04-09 - [Yielding the Event Loop in Large Synchronous Queues]
**Learning:** When using a concurrency pool like `pLimit` to replace `Promise.all` inside a large `for` loop (e.g. queueing tens of thousands of file searches), enqueuing all tasks synchronously can cause a massive memory spike and block the Node.js event loop. This also breaks dynamic early exits (like `if (results.length >= maxResults) break;`) because `results` never updates while the synchronous `for` loop occupies the main thread.
**Action:** When pushing thousands of promises into a `pLimit` queue, occasionally yield back to the event loop (e.g., `if (i % 50 === 0) await new Promise(r => setTimeout(r, 0));`) inside the loop. This throttles enqueue rate, keeps memory flat, and allows asynchronous tasks to complete and populate shared state (like `results.length`) so early exits work correctly.
