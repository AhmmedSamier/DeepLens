...
## 2026-06-01 - [Avoid O(n) early-exits inside helper functions in hot loops]
**Learning:** When applying early exits to O(N) hot loops, placing the early exit condition *inside* a helper function (like `calculateScore`) still incurs function call and variable assignment overhead for every item.
**Action:** Move simple boolean or bitmask early-exit checks to the very top of the calling loop method (like `processItemForSearch`) to skip function calls entirely for non-matching items.
## 2026-06-03 - [Inlining hot loops and ignoring complexity warnings]
**Learning:** If performance optimizations in hot loops (e.g., hoisting early-exit bitflag checks) cause a function to exceed the `sonarjs/cognitive-complexity` limit, it's better to bypass the rule rather than artificially splitting the code and re-introducing the function call overhead you set out to avoid.
**Action:** Use `// eslint-disable-next-line sonarjs/cognitive-complexity` to bypass the rule.
