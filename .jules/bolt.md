...
## 2026-08-21 - [Fast Newline Counting in Hot Paths]
**Learning:** In V8 (Node.js/Bun), using `while ((index = buffer.indexOf('
', lastIndex)) !== -1)` to count newlines in large strings incurs significant overhead in hot paths.
**Action:** Find the last newline with `buffer.lastIndexOf('
')` and use a fast native `buffer.charCodeAt(i) === 10` loop up to that index to count newlines. This is measurably faster for large buffers.
