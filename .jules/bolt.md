...
## 2026-08-10 - [O(1) Early-Exits outside of helper functions]
**Learning:** Even after implementing O(1) bitflag early-exits within property-specific helper functions (like `tryFuzzyMatchName`, `tryFuzzyMatchFullName`, `tryFuzzyMatchPath`), the main hot loop (`calculateFuzzyScore`) still incurs significant overhead simply by calling those functions for immediately rejected items. Function calls in JavaScript are relatively expensive when invoked millions of times during a rapid keystroke fuzzy search.
**Action:** Hoist property-specific O(1) bitflag checks out of the helper functions and into the main calling method (`calculateFuzzyScore`). Use boolean short-circuiting (`&&` or `? :`) to completely avoid invoking the helper functions for non-matching items.
