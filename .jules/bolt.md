...
## 2026-08-11 - [Class-Level Reusable Buffers]
**Learning:** For data dependent on method arguments in hot paths, using global constants isn't safe, but allocating objects per call causes high GC overhead.
**Action:** Use class-level reusable buffers (e.g., reusablePriorityTypeIds) that are cleared and repopulated per call. Because Node.js is single-threaded and SearchEngine searches are sequential (enforced by `_isSearching`), this safely avoids allocations. Always clear buffers at the beginning of the method to prevent state corruption from previous exceptions.
