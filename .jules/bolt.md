...
## 2024-05-14 - Hoist Object Allocations in Search Engine
**Learning:** During exploration of the codebase, I noticed that `new Uint8Array(256)` and `new Set<string>()` were being created on every keystroke within burst search loops (`searchRemainingItems` and `addUrlMatches`). This causes unnecessary GC overhead.
**Action:** Always check hot paths and loops for inline object/array instantiations and hoist them to module-level constants or class-level reusable properties, especially in search engines.
