...
## 2026-11-20 - [Avoid Object Allocation in Hot Loop Routing Evaluation]
**Learning:** Returning objects like `{ score: number, scope: SearchScope } | null` from functions called within hot loops (like `tryUrlEndpointMatch` and `calculateUrlMatchScore`) causes severe memory churn and unnecessary garbage collection overhead.
**Action:** Refactor internal hot path functions to return primitive scalars (e.g., `number` for score, returning `-Infinity` for failure) and handle scope assignment dynamically at the call site.
