...
## 2026-10-27 - [O(1) Path Property Early-Exit in Fuzzy Match]
**Learning:** Just like name and fullName properties, the path property is also a common target for fuzzy string matching in the fallback path. Without an explicit bitflag check for the path property, Fuzzysort.single() is evaluated unnecessarily.
