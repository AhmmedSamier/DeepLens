...
## 2024-11-20 - Hoisting property-specific bitflag checks in hot loop
**Learning:** Placing property-specific bitmask checks directly inside helper functions (like `tryFuzzyMatchName`) introduces unavoidable function call overhead for every immediately rejected item. This overhead adds up significantly in hot loop operations like `processItemForSearch`.
**Action:** When implementing O(1) early-exit checks (like bitmask validations) before executing expensive algorithms in a hot loop, hoist those checks directly into the calling loop to bypass function call overhead entirely for rejected items.
