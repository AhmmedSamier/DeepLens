...
## 2026-09-07 - [Fast Indentation Calculation]
**Learning:** In string processing hot paths, calculating indentation by finding the first non-whitespace character using regex (`str.search(/\S|$/)`) incurs significant regex engine overhead in V8/Node.js.
**Action:** Replace `str.search(/\S|$/)` with `str.length - str.trimStart().length`. This leverages fast V8 native methods for string manipulation, avoiding regex entirely and providing a measurable performance boost while maintaining full Unicode/BOM compatibility.
