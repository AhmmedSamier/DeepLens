...
## 2024-06-25 - ARIA Combobox for Webview Search Inputs
**Learning:** Implementing the ARIA Combobox pattern for search inputs in custom Webviews significantly improves screen reader accessibility. Missing `role="combobox"`, `aria-expanded`, and `aria-activedescendant` leads to screen readers being unable to announce selected options during keyboard navigation.
**Action:** Always pair a search input with `role="combobox"`, `aria-expanded`, and `aria-activedescendant` linked to a `role="listbox"` container with `role="option"` items, ensuring dynamic state like `aria-expanded` and `aria-activedescendant` are updated programmatically. Furthermore, the selection index logic must reach `-1` to allow users to properly clear the selection and return focus to the input field, avoiding a hard-stop at index `0`.
