...
## 2024-08-01 - ARIA Combobox for Webview Search
**Learning:** Custom hybrid keyboard/mouse listboxes in Webviews (like the search dropdown) that use custom arrow key navigation fail accessibility checks if the input isn't linked to the results. `role="combobox"`, dynamic `aria-expanded`, and `aria-activedescendant` are required for screen readers to announce highlighted options.
**Action:** Always implement the ARIA combobox pattern on custom auto-complete search inputs, ensuring `ArrowUp` allows navigating back to index `-1` to restore natural focus to the input.
