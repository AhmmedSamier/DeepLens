...
## 2025-02-14 - Prevent Focus Stealing in Search View
**Learning:** In hybrid mouse/keyboard search interfaces (like VS Code Webviews), interactive elements like filter buttons can steal DOM focus from the primary search input when clicked.
**Action:** Call `e.preventDefault()` on their `mousedown` event to prevent focus stealing while preserving click functionality.
