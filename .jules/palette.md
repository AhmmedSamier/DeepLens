...
## 2026-07-04 - Prevent Filter Buttons from Stealing Input Focus
**Learning:** In hybrid mouse/keyboard search interfaces (like VS Code Webviews), interactive elements like filter buttons can unintentionally steal DOM focus from the primary search input when clicked, forcing the user to click back into the input to continue typing.
**Action:** When implementing clickable filters or action buttons alongside a primary text input, call `e.preventDefault()` on their `mousedown` event to prevent them from stealing focus, ensuring a seamless typing and filtering experience.
