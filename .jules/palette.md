...
## 2024-11-20 - Webview Combobox Accessibility Pattern
**Learning:** In hybrid mouse/keyboard search interfaces like custom Webviews, custom arrow-key navigation often breaks screen reader compatibility. A custom list without `role="combobox"` and `aria-activedescendant` fails to announce the highlighted item when the user navigates with `ArrowDown`/`ArrowUp`. Additionally, hard-stopping `ArrowUp` at index `0` traps the user and prevents them from returning to the input field to type freely or see the original query without an active selection.
**Action:** When implementing arrow key navigation for a search input, always use the ARIA Combobox pattern (`role="combobox"`, `aria-expanded`, `aria-controls`, `aria-activedescendant` paired with `role="listbox"` and `role="option"`). Furthermore, ensure the `ArrowUp` selection logic allows the index to reach `-1` (e.g., `Math.max(selectedIndex - 1, -1)`) so the user can properly clear the selection.
