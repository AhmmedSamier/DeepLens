...
## 2026-08-07 - Implement ARIA Combobox Pattern for Custom Search Webview
**Learning:** Custom dropdown/search interfaces in VS Code webviews require explicit implementation of the WAI-ARIA Combobox pattern to be usable by screen readers. Key attributes include `role="combobox"` on the input, `role="listbox"` on the results container, `role="option"` with unique IDs on items, and dynamic state management for `aria-expanded`, `aria-selected`, and `aria-activedescendant`. A common keyboard navigation pitfall is trapping focus at the first element instead of allowing the ArrowUp key to clear the selection (setting the index to -1) and return logical focus back to the input field.
**Action:** Always implement the full ARIA Combobox suite for custom search inputs. Specifically, ensure the `ArrowUp` key handler uses `Math.max(selectedIndex - 1, -1)` (not `0`) to permit unselecting items, and ensure `aria-activedescendant` is correctly added and removed in sync with the index state.
