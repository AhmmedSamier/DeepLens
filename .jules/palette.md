...
## 2026-08-16 - Implement ARIA Combobox pattern for custom Search views
**Learning:** Custom input-based dropdown menus in Webviews need explicit ARIA combobox implementations (combobox, listbox, options, expanded state, and activedescendant) for screen readers to announce highlighted results as the user navigates with arrow keys.
**Action:** When implementing custom search UIs or dropdowns, ensure the full ARIA combobox pattern is implemented.
