...
## 2024-05-24 - Search View Focus Management
**Learning:** In hybrid mouse/keyboard search interfaces, interactive elements like filter buttons or list items naturally steal DOM focus from the primary search input when clicked, interrupting the user's typing flow.
**Action:** Prevent focus stealing by calling `e.preventDefault()` on the `mousedown` event of secondary interactive elements (filters, list items, action buttons) to keep focus locked on the primary search input.
