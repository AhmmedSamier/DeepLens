...
## 2024-10-27 - ARIA Combobox Pattern for Custom Search Inputs
**Learning:** Implementing the ARIA Combobox pattern manually requires careful management of dynamic state. Adding `role="combobox"` isn't enough; you must also dynamically update `aria-expanded` (true/false) based on results visibility, manage `aria-activedescendant` to reflect the currently focused item's ID, and ensure all result items possess `role="option"` with unique IDs.
**Action:** When creating custom search inputs, always pair `role="combobox"` on the input with `role="listbox"` on the container, and use JavaScript to dynamically sync `aria-expanded`, `aria-activedescendant`, and `role="option"` states based on the UI.
