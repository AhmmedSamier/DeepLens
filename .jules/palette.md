...
## 2024-08-01 - Allow deselecting items in ARIA Combobox
**Learning:** When navigating an ARIA combobox with arrow keys, hard-stopping at index 0 prevents users from clearing their selection to return focus fully to the text input context. Screen readers rely on `aria-activedescendant` removal to announce that focus has returned to the input field itself.
**Action:** Always ensure `ArrowUp` selection logic allows the index to reach `-1` (e.g., `Math.max(selectedIndex - 1, -1)`) to clear the selection and properly remove the `aria-activedescendant` attribute.
