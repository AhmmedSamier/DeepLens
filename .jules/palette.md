...
## 2024-05-31 - Search Input Accessibility with ARIA Combobox
**Learning:** When creating custom search interfaces with dropdown results, `role="combobox"`, `aria-expanded`, and `aria-activedescendant` on the input, paired with a `role="listbox"` on the container and `role="option"` with unique IDs on the results are necessary for screen readers to correctly announce highlighted items.
**Action:** Apply this ARIA Combobox pattern to all hybrid mouse/keyboard search inputs to ensure screen reader users receive navigation feedback.
