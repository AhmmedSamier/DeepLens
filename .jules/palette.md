...
## 2026-07-02 - ARIA Combobox Pattern for Search Inputs
**Learning:** When implementing arrow key navigation for a search input in custom Webviews without standard VSCode webview controls, using only visual highlighting (`.selected` classes) makes the navigation invisible to screen readers.
**Action:** Use the ARIA Combobox pattern: apply `role="combobox"`, `aria-expanded` (dynamic), and `aria-activedescendant` (dynamic) to the input, pair it with a `role="listbox"` container, give each result item `role="option"` with a unique ID, and allow selection index to reach -1 when clearing the list selection with ArrowUp.
