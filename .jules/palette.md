...
## 2026-08-26 - Allow clearing selection with ArrowUp
**Learning:** When implementing custom keyboard navigation for UIs like ARIA Comboboxes, hard-stopping 'ArrowUp' at index 0 prevents users from fully clearing their selection to return pure focus to the input field.
**Action:** Ensure ArrowUp logic allows the index to reach -1 (e.g., `Math.max(selectedIndex - 1, -1)`), thereby un-selecting all options.
