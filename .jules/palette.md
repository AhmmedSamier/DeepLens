...
## 2026-08-23 - Fix ARIA Combobox implementation and ArrowUp selection
**Learning:** When implementing the ARIA Combobox pattern (e.g., adding `role="combobox"`), avoid accessibility anti-patterns by ensuring dynamic state is properly managed. You must update `aria-expanded` (true/false) based on results visibility, dynamically manage `aria-activedescendant`, and ensure all result container items possess `role="option"` with unique IDs. Furthermore, when implementing custom keyboard navigation for ARIA Comboboxes, ensure that `ArrowUp` selection logic allows the index to reach `-1` (e.g., `Math.max(selectedIndex - 1, -1)`) so the user can properly clear the selection and return focus to the input field, rather than hard-stopping at index `0`.
**Action:** Always maintain `aria-expanded` and `aria-activedescendant` accurately, ensure all list items have `role="option"` and unique IDs, and allow keyboard navigation to reach `-1` to clear active descendant selection.
