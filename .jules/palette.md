...
## 2026-09-02 - Prevent Focus Stealing in Empty State Buttons
**Learning:** In hybrid mouse/keyboard search interfaces, clicking interactive empty state or history buttons steals DOM focus from the primary search input, forcing users to manually click back into the input or hit Tab before they can resume typing.
**Action:** When implementing actionable buttons in empty states or toolbars that don't need their own text-input focus, attach a `mousedown` event listener that calls `e.preventDefault()`. This ensures the user's cursor remains firmly anchored in the main search input.
