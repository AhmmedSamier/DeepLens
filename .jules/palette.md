...
## 2026-07-30 - Search Input ARIA Combobox Pattern
**Learning:** When implementing a search input that displays a dropdown of results in custom Webviews, using the ARIA Combobox pattern ensures screen readers correctly announce the highlighted item during keyboard navigation. This requires setting `role="combobox"`, dynamically managing `aria-expanded` and `aria-activedescendant` on the input, and ensuring result items have `role="option"` with unique IDs within a `role="listbox"`.
**Action:** Always apply the full ARIA Combobox pattern (`role="combobox"`, `aria-expanded`, `aria-activedescendant`, `role="listbox"`, `role="option"`) to custom search inputs with dropdown results, and ensure `ArrowUp` navigation allows selecting an index of `-1` to properly clear the selection and return focus to the input field.
