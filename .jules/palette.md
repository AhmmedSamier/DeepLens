...
## 2026-06-30 - Fix Interactive Elements Stealing Focus
**Learning:** In hybrid mouse/keyboard search interfaces (like VS Code Webviews), interactive elements like list items or action buttons can steal DOM focus from the primary search input when clicked, disrupting the keyboard workflow.
**Action:** Always call `e.preventDefault()` on the `mousedown` event for these interactive elements to prevent focus stealing while still allowing the `click` event to register.
