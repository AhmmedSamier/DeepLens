...
## 2024-08-06 - ARIA Combobox Arrow Navigation Pattern
**Learning:** When implementing custom keyboard navigation for ARIA Comboboxes, hard-stopping `ArrowUp` at index 0 traps focus inside the listbox, violating a11y expectations. Users must be able to return to a completely unselected state (-1) to safely clear selection or type without modifying the active item.
**Action:** Always ensure list navigation logic (e.g., `Math.max(index - 1, -1)`) allows the active index to reset to `-1` and removes `aria-activedescendant` when returning to the input.
