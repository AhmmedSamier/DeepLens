...
## 2026-08-22 - ARIA Combobox implementation
**Learning:** Implementing the ARIA Combobox pattern requires dynamically updating aria-expanded based on results visibility, dynamically managing aria-activedescendant, and ensuring all result container items possess role="option" with unique IDs. Moreover, custom keyboard navigation for ARIA Comboboxes must ensure that ArrowUp selection logic allows the index to reach -1 to properly clear the selection and return focus conceptually to the input field.
**Action:** Use this as a reference for future ARIA combobox implementations.
