...
describe("RouteMatcher", () => {
    ...
    it("should match parameterized paths", () => {
        ...
        expect(RouteMatcher.isMatch("api/AdminDashboard/customers/{customerId}", "api/AdminDashboard/customers/5")).toBe(true);
        // Partial match (no api/ prefix)
        expect(RouteMatcher.isMatch("api/AdminDashboard/customers/{customerId}", "AdminDashboard/customers/5")).toBe(true);
    });

    it("should match multiple parameters", () => {
        expect(RouteMatcher.isMatch("api/companies/{companyId}/employees/{employeeId}", "api/companies/1/employees/42")).toBe(true);
    });
    ...
    it("should handle incomplete paths (fuzzy match)", () => {
        ...
        expect(RouteMatcher.isMatch("api/AdminDashboard/customers/{id}/{id2}/username", "AdminDashboard/customers/5/10/usern")).toBe(true);
    });
});
