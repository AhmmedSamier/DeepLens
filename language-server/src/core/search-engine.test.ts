...
describe("SearchEngine", () => {
  ...
  it("should handle filename:line pattern", async () => {
    ...
    const classMatch = results.find(
      (r) => r.item.type === SearchItemType.CLASS,
    );
    if (classMatch) {
...
describe("SearchEngine incremental update deduplication", () => {
  ...
  it("should have correct filePaths after remove-then-add (simulating file change)", async () => {
    ...
    const symbol = createTestItem(
      "sym:src/Bar.ts:MyClass",
      "MyClass",
      SearchItemType.CLASS,
      "src/Bar.ts",
    );
    ...
    const newFileItem = createTestItem(
      "file:src/Bar.ts",
      "Bar.ts",
      SearchItemType.FILE,
      "src/Bar.ts",
    );
    const newSymbol = createTestItem(
      "sym:src/Bar.ts:MyClass",
      "MyClass",
      SearchItemType.CLASS,
      "src/Bar.ts",
...
describe("RealIndexSearch", () => {
  it("should find 3 EmployeeService classes with lowercase query (backend, frontend, admin)", async () => {
    ...
    const esClasses = results.filter(
      (r) =>
        r.item.type === SearchItemType.CLASS &&
        r.item.name === "EmployeeService",
    );
  ...
  describe("VS Code extension integration simulation", () => {
    ...
    it("should return all 3 EmployeeService classes in TYPES scope through all phases", async () => {
      ...
      const backEndPath = (c: { item: { filePath: string } }) =>
        c.item.filePath.includes("BackEnd");
      const frontEndPath = (c: { item: { filePath: string } }) =>
        c.item.filePath.includes("FrontEnd");
      const adminPath = (c: { item: { filePath: string } }) =>
        c.item.filePath.includes("admin");
