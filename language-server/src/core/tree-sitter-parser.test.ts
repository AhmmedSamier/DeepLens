...
namespace WebAPI.Controllers
{
    ...
    try {
      ...
      const endpoints = items.filter(
        (item) => item.type === SearchItemType.ENDPOINT,
      );
      ...
      const getRevenue = endpoints.find((e) =>
        e.fullName.includes("GetRevenueSummary"),
      );
      expect(getRevenue).toBeDefined();
      expect(getRevenue?.name).toContain("api/AdminDashboard/{year}");

      const getAllCustomers = endpoints.find((e) =>
        e.fullName.includes("GetAllCustomers"),
      );
      expect(getAllCustomers).toBeDefined();
      expect(getAllCustomers?.name).toContain("api/AdminDashboard/customers");

      const changeUsername = endpoints.find((e) =>
        e.fullName.includes("ChangeEmployeeUsername"),
      );
  ...
  it("should detect endpoints in the user's provided controller", async () => {
    ...
    const testFilePath = path.join(
      __dirname,
      "UserDashboardController.auto.cs",
    );
    const csharpCode = `
using Helpers;
using Microsoft.AspNetCore.Mvc;
