...
export class SearchEngine implements ISearchProvider {
  ...
  private readonly preparedCache: Map<
    string,
    { prepared: Fuzzysort.Prepared; refCount: number }
  > = new Map();
  ...
  private readonly fileItemByNormalizedPath: Map<string, SearchableItem> =
    new Map();
  ...
  async addItems(items: SearchableItem[]): Promise<void> {
    ...
    if (this.itemTypeIds.length < requiredSize) {
      ...
      const newCapacity = Math.max(
        requiredSize,
        Math.ceil(this.itemTypeIds.length * 1.5),
      );
  ...
  removeItemsByFiles(filePaths: string[]): void {
    ...
    const { indicesToRemove, normalizedRemovedPaths } =
      this.identifyIndicesToRemove(filePaths);
  ...
  private prepareItemAtIndex(item: SearchableItem, index: number): void {
    ...
    const { nameFlags, fullNameFlags, pathFlags, aggregateFlags } =
      this.computeItemBitflags(item);
  ...
  private shouldProcessFullName(item: SearchableItem): boolean {
    return (
      ...
      item.fullName !== item.relativeFilePath
    );
  }
  ...
  private populateParallelArrays(item: SearchableItem): void {
    ...
    this.preparedFullNames.push(
      shouldPrepareFullName && item.fullName
        ? this.getPrepared(item.fullName)
        : null,
    );
    ...
    this.preparedPatterns.push(
      item.type === SearchItemType.ENDPOINT
        ? RouteMatcher.precompute(item.name)
        : null,
    );
  }
  ...
  private createSearchContext(options: SearchOptions): SearchContext {
    ...
    return {
      ...
      normalizedQuery:
        query.indexOf("\\") !== -1 ? query.replace(/\\/g, "/") : query,
      queryUpper,
      queryLower,
  ...
  async search(
    ...
    const onResult =
      typeof onResultOrToken === "function" ? onResultOrToken : undefined;
    // If the second arg is the token, use it. Otherwise use the third arg.
    const cancellationToken = onResult
      ? token
      : (onResultOrToken as CancellationToken);
  ...
  private async executeProviderSearch(
    ...
    for (let i = 0; i < providerStatuses.length; i++) {
      ...
      if (status.status === "rejected") {
        this.logger?.error(
          `Provider ${this.providers[i].id} failed: ${status.reason}`,
        );
      }
    }
  ...
  private async getIndicesForModifiedFiles(): Promise<number[]> {
    ...
    if (
      ...
      this.modifiedIndicesCache.expiresAt > now
    ) {
  ...
  private async performTextSearch(
    ...
    if (this.ripgrep?.isAvailable()) {
      const results = await this.performRipgrepSearch(
        query,
        maxResults,
        onResult,
        token,
  ...
  private async performRipgrepSearch(
    ...
    try {
      ...
      const matches = await this.ripgrep.search(
        query,
        this.filePaths,
        maxResults,
        token,
      ...
      for (const match of matches) {
        ...
        if (!fileItem) {
          fileItem = this.fileItemByNormalizedPath.get(
            this.normalizePath(match.path),
          );
        }

        if (!fileItem) {
          this.logger?.error(
            `Ripgrep result path not found in index: ${match.path}`,
          );
          continue;
        }
      ...
      this.logger?.error(
        `Ripgrep failed: ${error}. Falling back to Node.js stream.`,
      );
      return null;
    }
  }
  ...
  private async performStreamSearch(
    ...
    this.logger?.log(
      `--- Starting LSP Text Search (Streaming): "${query}" ---`,
    );
    const startTime = Date.now();
    const results: SearchResult[] = [];
    ...
    for (
      let start = 0;
      start < prioritizedFiles.length;
      start += currentConcurrency
    ) {
  ...
  private async scanFileStream(
    ...
    return new Promise<void>((resolve) => {
      ...
      stream.on("data", (chunk: string) => {
        ...
        const completeLine =
          chunkBuffer.length > 0 ? chunkBuffer.join("") + firstPart : firstPart;
  ...
  private processBufferLines(
    ...
    if (!nextMatch) {
      ...
      return this.advanceLinesWithoutMatches(
        buffer,
        bufferOffset,
        startLineIndex,
      );
  ...
  public async performSymbolSearch(
    context: SearchContext,
  ): Promise<SearchResult[]> {
    let indices: number[] | undefined;
  ...
  public async performFileSearch(
    context: SearchContext,
  ): Promise<SearchResult[]> {
    let indices: number[] | undefined;
  ...
  private performUnifiedSearch(
    ...
    const preferredIndices = this.getPreferredIndicesForQuery(
      scope,
      query,
      indices,
    );
    ...
    if (this.getActivityScore) {
      for (let i = 0; i < results.length; i++) {
        ...
        if (activityScore > 0 && r.score > 0.05) {
          r.score =
            r.score * (1 - this.activityWeight) +
            activityScore * this.activityWeight;
        }
      }
  ...
  private prepareSearchContext(query: string, scope: SearchScope) {
    ...
    const queryForUrlMatch = isPotentialUrl
      ? RouteMatcher.prepare(query)
      : query;
  ...
  private processItemForSearch(
    ...
    const passesBitflag =
      (context.itemBitflags[i] & context.queryBitflags) ===
      context.queryBitflags;
    ...
    if (!passesBitflag) {
      ...
      if (context.isPotentialUrl) {
        ...
        shouldPreserveEndpointRouteMatch =
          typeId === TYPE_TO_ID[SearchItemType.ENDPOINT];
      }
    ...
    let score = passesBitflag
      ? this.calculateSearchScore(i, typeId, context)
      : -Infinity;
  ...
  private calculateFuzzyScore(
    ...
    const fullNameScore =
      nameScore < 0.9 ? this.tryFuzzyMatchFullName(i, context) : -Infinity;
    const bestNameOrFull =
      fullNameScore > nameScore ? fullNameScore : nameScore;

    // Try matching against path (weight: 0.8) if still not high enough
    const pathScore =
      bestNameOrFull < 0.8 ? this.tryFuzzyMatchPath(i, context) : -Infinity;
  ...
  private tryFuzzyMatchName(
    ...
    if (
      (context.itemNameBitflags[i] & context.queryBitflags) !==
      context.queryBitflags
    ) {
      return -Infinity;
    }
  ...
  private tryFuzzyMatchFullName(
    ...
    if (
      (context.itemFullNameBitflags[i] & context.queryBitflags) !==
      context.queryBitflags
    ) {
      return -Infinity;
    }
  ...
  private calculateUrlMatchScore(
    ...
    const qMethod =
      typeof context.queryForUrlMatch === "string"
        ? undefined
        : context.queryForUrlMatch.method;
    ...
    if (finalQueryForMatch) {
      const urlScore = RouteMatcher.scoreMatchPattern(
        pattern,
        finalQueryForMatch,
      );
      if (urlScore > 0) {
  ...
  private computeActivityScore(
    ...
    if (currentScore > minScore && this.getActivityScore) {
      ...
      if (activityScore > 0 && currentScore > 0.05) {
        return (
          currentScore * (1 - this.activityWeight) +
          activityScore * this.activityWeight
        );
      }
    }
  ...
  private calculateItemScore(
    ...
    const fullNameScore = this.scoreField(
      query,
      preparedFullName,
      queryLen,
      minScore,
  ...
  private findBurstMatches(
    ...
    if (indices) {
      ...
      this.searchAllScopesInPriorityOrder(
        maxResults,
        processItem,
        results,
        token,
  ...
  private applyPersonalizedBoosting(results: SearchResult[]): void {
    ...
    for (const result of results) {
      ...
      if (activityScore > 0) {
        ...
        if (baseScore > 0.05) {
          result.score =
            baseScore * (1 - this.activityWeight) +
            activityScore * this.activityWeight;
        }
      }
    }
