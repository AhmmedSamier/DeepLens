...
export class SearchEngine implements ISearchProvider {
    ...
    private processItemForSearch(
        ...
        if (context.isPotentialUrl && typeId === 11 /* ENDPOINT */) {
            const urlResultScore = this.tryUrlEndpointMatch(i, typeId, context, score);
            if (urlResultScore > score) {
                score = urlResultScore;
    ...
    private tryUrlEndpointMatch(
        ...
        const matchResultScore = this.calculateUrlMatchScore(pattern, context);
        if (matchResultScore > currentScore) {
