...
describe("RouteMatcher Regression Tests", () => {
  ...
  it("should handle different methods correctly", () => {
    ...
    expect(
      RouteMatcher.scoreMatch(template, "post api/data/123"),
    ).toBeGreaterThan(0);
    expect(
      RouteMatcher.scoreMatch(template, "DELETE api/data/123"),
    ).toBeGreaterThan(0);
    expect(
      RouteMatcher.scoreMatch(template, "put /api/data/123"),
    ).toBeGreaterThan(0);
  });
  ...
  it("should distinguish between exact matches and loose suffix matches even with methods", () => {
    ...
    const irrelevantRoute =
      "api/Roles/{PageNumber}/{PageSize}/{SortBy}/{SortDirection}";
