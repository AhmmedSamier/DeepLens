...
[46, 42, 43, 63, 94, 36, 123, 125, 40, 41, 124, 91, 93, 92].forEach(
  (c) => (REGEX_SPECIAL_CHARS[c] = 1),
);
...
export class RouteMatcher {
  ...
  static prepare(concretePath: string): PreparedPath {
    ...
    const end =
      cleanPath.charCodeAt(cleanPath.length - 1) === 47
        ? cleanPath.length - 1
        : cleanPath.length;
   ...
   */
  static scoreMatchPattern(
    pattern: RoutePattern,
    concretePath: string | PreparedPath,
  ): number {
    ...
    try {
      ...
      if (
        pattern.hasCatchAll ||
        pathSegments.length === pattern.templateSegments.length
      ) {
      ...
      if (matchesRegex) {
        ...
        const count = Math.min(
          pathSegments.length,
          pattern.templateSegments.length,
        );
  ...
  private static getOrCompileCache(template: string): RoutePattern | null {
    ...
    if (trimmedTemplate.charCodeAt(0) === 91) {
      ...
      if (closeIdx > 1) {
        ...
        if (method) {
          ...
          while (
            start < trimmedTemplate.length &&
            trimmedTemplate.charCodeAt(start) === 32
          ) {
            // ' '
            start++;
          }
    ...
    const end =
      cleanTemplate.charCodeAt(cleanTemplate.length - 1) === 47
        ? cleanTemplate.length - 1
