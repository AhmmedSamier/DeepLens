...
export class WorkspaceIndexer {
    ...
    private async indexFileSymbols(filePath: string, version?: number): Promise<void> {
        try {
            await this.treeSitter.init();
            const items = await this.treeSitter.parseFile(filePath);

            // Check if this result is stale (file has changed again since we started)
    ...
    private async handleFileCreated(filePath: string): Promise<void> {
        ...
        if (filePath.endsWith('.cs') && (await this.isAutoGeneratedFile(filePath))) {
            return;
        }
        ...
        // Index symbols
        await this.indexFileSymbols(filePath, version);
    ...
    private async handleFileChanged(filePath: string): Promise<void> {
        ...
        // Increment version to invalidate previous pending index operations for this file
