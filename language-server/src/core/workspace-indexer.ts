...
export class WorkspaceIndexer {
  ...
  private readonly gitCheckResolvers: Map<
    string,
  ...
  private getWorkers(): Worker[] {
    ...
    if (!workerScript) {
      this.log(
        `ERROR: Worker script not found. Searched in: ${possibleScripts.join(", ")}`,
      );
      return [];
    }
  ...
  public onItemsAdded(listener: (items: SearchableItem[]) => void) {
    ...
    return {
      dispose: () => {
        this.onItemsAddedListeners = this.onItemsAddedListeners.filter(
          (l) => l !== listener,
        );
      },
    };
  ...
  public onItemsRemoved(listener: (filePath: string) => void) {
    ...
    return {
      dispose: () => {
        this.onItemsRemovedListeners = this.onItemsRemovedListeners.filter(
          (l) => l !== listener,
        );
      },
    };
  ...
  async indexWorkspace(
    ...
    try {
      ...
      this.log(
        `Step 3/4: Extracting symbols from ${fileItems.length} files...`,
      );
        ...
        try {
          ...
          while (lastIndex < len) {
            ...
            if (lastIndex < nullIndex) {
              ...
              const fullPath = path.isAbsolute(line)
                ? line
                : path.join(folderPath, line);
          ...
          console.debug(
            `Git file listing failed for ${folderPath}: ${errorMessage}`,
          );
          return [];
        }
      }),
    ...
    while (attempts < 3) {
      try {
        ...
        if (
          ...
          if (attempts >= 3) {
            this.log(
              `Failed to get file size for ${filePath} after 3 attempts: ${err.message}`,
            );
            return undefined;
          }
          // Exponential backoff: 20ms, 80ms
          await new Promise((resolve) =>
            setTimeout(resolve, 20 * Math.pow(4, attempts - 1)),
          );
        } else {
  ...
  private async isAutoGeneratedFile(filePath: string): Promise<boolean> {
    ...
    try {
      ...
      return (
        ...
        content.indexOf("<auto-generated") !== -1 // Some files have it later or in a header block
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error
          ...
          : JSON.stringify(error) || String(error);
      console.debug(
        `Auto-generated check failed for ${filePath}: ${errorMessage}`,
      );
      return false;
    } finally {
      if (fd) {
        try {
          ...
          console.debug(
            `Failed to close file handle for ${filePath}: ${closeErrorMessage}`,
          );
        }
      }
    }
  ...
  private async scanWorkspaceSymbols(
    ...
    try {
      ...
      const timeoutPromise = new Promise<"timeout">((resolve) => {
        setTimeout(
          () => resolve("timeout"),
          WorkspaceIndexer.WORKSPACE_SYMBOL_TIMEOUT_MS,
        );
      });

      const resolved = await Promise.race([
        workspaceSymbolsPromise,
        timeoutPromise,
      ]);
      ...
      const errorMessage =
        error instanceof Error
          ...
          : JSON.stringify(error) || String(error);
      console.debug(
        `Workspace symbol pass failed, moving to file-by-file: ${errorMessage}`,
      );
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          internedItems[i] = {
            ...
            containerName: item.containerName
              ? this.intern(item.containerName)
              : undefined,
  ...
  private updateWorkerProgress(
    ...
    if (
      processed % 100 < itemsProcessed ||
      (processed === totalFiles && !logged100)
    ) {
    ...
    if (progressCallback) {
      ...
      if (percentage >= nextReportingPercentage || processed === totalFiles) {
        progressCallback(
          `Indexing batch... (${processed}/${totalFiles})`,
          percentage,
        );
  ...
  private processSymbols(
    ...
    for (const symbol of symbols) {
      ...
      if (itemType) {
        ...
        const internedContainerName = containerName
          ? this.intern(containerName)
          : undefined;
      ...
      if (symbol.children && symbol.children.length > 0) {
        const newContainerName = containerName
      ...
      fileItems.map((fileItem) =>
        limit(async () => {
          ...
          if (progressCallback) {
            const percentage = (processed / totalFiles) * 100;
            if (
              percentage >= nextReportingPercentage ||
              processed === totalFiles
            ) {
              progressCallback(
                `Indexing symbols... (${processed}/${totalFiles})`,
                percentage,
              );
  ...
  private setupFileWatchers(): void {
    ...
    this.fileWatcher = this.env.createFileSystemWatcher(
      pattern,
      (filePath, type) => {
  ...
  private async processGitCheckBatch(
    folder: string,
    filesArray: string[],
  ): Promise<void> {
    try {
  ...
  private async handleFileCreated(filePath: string): Promise<void> {
    ...
    if (
      ...
      this.isLikelyGeneratedCSharpPath(filePath)
    ) {
      return;
    }
  ...
  private getCachedString(str: string): string {
    ...
    if (this.stringCache.size >= WorkspaceIndexer.STRING_CACHE_MAX_SIZE) {
      ...
      const evictionCount = Math.max(
        1,
        Math.floor(WorkspaceIndexer.STRING_CACHE_MAX_SIZE * 0.1),
      );
