...
export class WorkspaceIndexer {
    ...
    private async processFileList(files: string[], collector: (items: SearchableItem[]) => void): Promise<void> {
        ...
        // NOTE: `batch` is a shared accumulator across all concurrent pLimit tasks.
        // JavaScript is single-threaded, so push() and the length check are atomic
        // within each synchronous block. However, tasks interleave at await points
        // (e.g. isAutoGeneratedFile), meaning multiple tasks may accumulate items
        // before any one of them reaches the drain threshold. This produces
        // non-deterministic batch sizes but is correct: no items are lost, and the
        // final unconditional flush after Promise.all guarantees full coverage.
    ...
    private handleFileDeleted(filePath: string): void {
        ...
        // NOTE: We intentionally skip the isGitIgnored() check here (unlike handleFileCreated
        // and handleFileChanged). On delete, the file no longer exists, so the gitignore
        // check would fail or be meaningless. More importantly, cleanup should always run:
        // if a gitignored file somehow ended up in the index (e.g. .gitignore was added
        // after initial indexing), we still want to evict it cleanly on deletion.
