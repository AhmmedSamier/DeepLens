...
describe("RouteMatcher", () => {
  ...
  it("should match parameterized paths", () => {
    expect(RouteMatcher.isMatch("api/customers/{id}", "api/customers/5")).toBe(
      true,
    );
    expect(
      RouteMatcher.isMatch("api/customers/{id}", "api/customers/abc"),
    ).toBe(true);
    expect(
      RouteMatcher.isMatch(
        "api/AdminDashboard/customers/{customerId}",
        "api/AdminDashboard/customers/5",
      ),
    ).toBe(true);
    expect(
      RouteMatcher.isMatch(
        "api/AdminDashboard/customers/{id}",
        "api/AdminDashboard/customers/50",
      ),
    ).toBe(true);
  ...
  it("should match paths with HTTP method prefixes", () => {
    expect(
      RouteMatcher.isMatch("[GET] api/customers/{id}", "api/customers/5"),
    ).toBe(true);
    expect(RouteMatcher.isMatch("[POST] api/orders", "api/orders")).toBe(true);
  });

  it("should handle optional parameters and constraints", () => {
    expect(RouteMatcher.isMatch("api/items/{id:int}", "api/items/123")).toBe(
      true,
    );
  ...
  it("should not match unrelated paths", () => {
    expect(RouteMatcher.isMatch("api/customers/{id}", "api/orders/5")).toBe(
      false,
    );
    expect(RouteMatcher.isMatch("api/customers", "api/customers/5")).toBe(
      false,
    );
  });
